                ...
                <Dialog header="Новое сканирование" @addProject="emitProjectAdd" v-model:visible="showAddVideo" :breakpoints="{ '960px': '75vw' }" :style="{ width: '30vw' }" :modal="true">
					...
					<div class="formgrid grid">
						<div class="field col">
                                ...
                                <FileUpload name="file[]" :customUpload="true" @uploader="importUploader" mode="basic" ref='file' accept="" :maxFileSize="50000000000" label="Видео" chooseLabel="Видео" class="ml-6" :auto="true"/>					
							</div>
							<div class="field col">
								<label for="frequency" class="ml-6">Загрузка субтитров</label>
			                    <FileUpload name="subtitles[]" :customUpload="true" @uploader="importSubsUploader" mode="basic" ref='subtitles' accept="" :maxFileSize="50000000000" label="Субтитры" chooseLabel="Субтитры" class="ml-6" :auto="true"/>							
							</div>
					</div>
                    ...
                    <template #footer>
					    <Button label="Начать сканирование" @click="addVideo" icon="pi pi-check" class="p-button-success" :disabled="isSendButton"/>
            ...
            <Toolbar class="mb-1">
                ...
                >
					...
					<template #body="{data}" class="">
					 ...
					 <p v-if="data.status === 'Начало работы' || data.status === 'Распознаем аномалии' || data.status === 'Кадрируем видео' || data.status === 'Выполнено' || data.status === 'Обрабатываем найденные аномалии'">
					 {{ data.status }}
					 </p>
					 <p v-else-if="data.status === 100">Выполнено</p>					 
					 <p v-else>
					 {{ data.status }}
					 <ProgressBar :value="data.status" :showValue="false" style="height: 1em;" class='m-3 mt-1'/>
					 </p>					   
					   <!--
					   ...
					   <!--ProgressBar v-if="data.status != 'Выполнено' && data.status != 'Начало работы' && data.status === 'Обрабатываем найденные аномалии'" :value="data.status" :showValue="false" style="height: 1em;" class='m-3 mt-1'/-->
					 </span>
					</template>
					  </Column>					
...
const watchVideoStatus = (colId, rowId, id) => {
  ...
  sockets[id] = useWebSocket(`ws://84.201.177.58/ws/ready/${id}/?token=${stateuser.token}`,{  
	...
	let status = (raw)? raw.complete : 9 //'Начало работы'
	if(status === 'Выполнено' || status === 100) {
	  projects.value[colId].video[rowId].status = 'Выполнено'
	  projects.value[colId].video[rowId].founded_anomaly = 100
	}
    else {	
	...
	if(status >= 10 || status < 100) projects.value[colId].video[rowId].status = status 
	if(status < 10) projects.value[colId].video[rowId].status = 'Начало работы' 
	/*
...
const addVideo = async () => {
  ...
  if(!formData.value.has('file')) return
  //if(!formData.value.has('subtitles')) return

  const message = 'Предупреждение! Для корректной обработки загружаемых файлов: не обновляйте и не закрывайте страницу нашего сервиса в браузере, а также не переходите в другие функции сервиса.'
  toast.add({severity: 'warn', summary: message, detail: '', life: 6000})
  loading.value = true
